...
#SL<-SL%>%dyplr::filter(Location_Category_Description %in% c('Elementary','High school','Junior High-Intermediate-Middle','K-8'))
...
shinyServer(function(input, output) {
  ...
  filteredData <- reactive({
    ...
    SL %>% dyplr::filter(Level %in% selected_schoollevel) 
  })
